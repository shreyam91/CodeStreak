...
const placementTopics = [
  { id: 1, title: "Operating Systems", difficulty: "Medium", category: "CS Core", status: "solved" },
  { id: 2, title: "Computer Networks", difficulty: "Medium", category: "CS Core", status: "unsolved" },
  { id: 3, title: "HR & Behavioral", difficulty: "Easy", category: "Soft Skills", status: "solved" },
  { id: 4, title: "DBMS", difficulty: "Medium", category: "CS Core", status: "unsolved" },
  { id: 5, title: "System Fundamentals", difficulty: "Hard", category: "CS Core", status: "unsolved" },
  { id: 6, title: "Leadership Principles", difficulty: "Easy", category: "Soft Skills", status: "unsolved" },
];
...
export default function PlacementPrepDashboardPage() {
  ...
  const categories = ["All", "CS Core", "Soft Skills", "Aptitude"];

  const filteredTopics = placementTopics.filter(t => {
              ...
              <div className="border border-border bg-card rounded-2xl shadow-sm overflow-hidden">
                ...
                <div className="divide-y divide-border">
                  {filteredTopics.map((topic) => (
                    <Link 
                      key={topic.id} 
                      href={`/dashboard/placement-prep/${topic.id}`}
                    ...
                    >
                      ...
                      <div className="col-span-2">
                        <span className={`text-xs px-2.5 py-1 rounded-md font-semibold ${
                          ...
                          topic.difficulty === 'Medium' ? 'bg-yellow-500/10 text-yellow-500' : 'bg-red-500/10 text-red-500'
                        }`}>
                          {topic.difficulty}
                        </span>
                      </div>
                      <div className="col-span-3 text-sm text-muted-foreground">{topic.category}</div>
                    </Link>
                  ))}
                </div>
              </div>
